...
* 終端機會直接顯示容器內的運作日誌
```
docker run -p 8080:8000 --name running-app --env PORT=8000 bike-app
```
...
1. 系統會根據輸入產出具體的租借數量預估。例如：

* **情境**：5月 (春季)、週一、08:00 (通勤尖峰)、20°C、晴天。
* **AI 分析結果**：約 **2,061 輛**。

2. 使用 Chart.js 這個輕量級的 JavaScript庫繪製出折線圖。

- **視覺化趨勢**：使用者可以連續點擊不同的溫度或小時，觀察圖表中的波峰波谷，直觀看出資料是否規律。
